...
async function approveSubmission(id){
  ...
  if(s.merchant_name){
    const {data:m}=await db.from('merchants').select('id').ilike('name',s.merchant_name).limit(1);
    if(m?.length)merchantId=m[0].id;else{
      const slug=slugify(s.merchant_name)+'-'+Date.now().toString().slice(-5);
      const {data:newM,error:me}=await db.from('merchants').insert({name:s.merchant_name,slug,active:true}).select('id').single();
      if(me){alert(me.message);return;}merchantId=newM.id;
    }
  }
  ...
  const contentHtml=s.description?'<p>'+esc(s.description).replace(/\n/g,'</p><p>')+'</p>':'';
  ...
  const {data:newDeal,error:de}=await db.from('deals').insert(deal).select('id').single();
  if(de){alert(de.message);return;}
  const {error:ue}=await db.from('offer_submissions').update({status:'approved',approved_deal_id:newDeal.id,reviewed_at:new Date().toISOString()}).eq('id',id);
...
async function rejectSubmission(id){
  ...
  const {error}=await db.from('offer_submissions').update({status:'rejected',admin_note:note,reviewed_at:new Date().toISOString()}).eq('id',id);
  if(error)alert(error.message);else loadSubmissionQueue();
}
