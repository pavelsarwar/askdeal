let offerEditor=null,offerEditingId=null;

async function showOffersCMS(){
  moduleCard.innerHTML=`<div class="deals-head"><div><h2 style="margin:0">Offer Posts</h2><div class="result-count">Create and publish offers like a WordPress post</div></div><button class="btn btn-primary" onclick="openOfferEditor()">+ Add New Offer</button></div><div id="offerRows">Loading...</div>`;
...
async function openOfferEditor(record=null){
  offerEditingId=record?.id||null;
  const [cats,merchants,states]=await Promise.all([
    db.from('categories').select('id,name').eq('active',true).order('name'),
    db.from('merchants').select('id,name').eq('active',true).order('name'),
    db.from('states').select('id,name').order('name')
  ]);
  moduleCard.innerHTML=`
  <div class="deals-head"><div><h2 style="margin:0">${record?'Edit':'Add'} Offer</h2><div class="result-count">Write, format and publish an offer</div></div><button class="btn btn-light" onclick="showOffersCMS()">← Back</button></div>
  <form id="offerForm" onsubmit="saveOffer(event)" class="offer-editor-layout">
    <div class="offer-main">
      <div class="field"><label>Title</label><input id="offerTitle" required value="${attr(record?.title||'')}" placeholder="e.g. AEON Weekend Grocery Sale"></div>
      <div class="field"><label>Details</label><div id="offerRichEditor"></div><div class="hint">Use headings, bold, lists, links and the image button. Images are uploaded to Supabase Storage.</div></div>
      <div class="field"><label>Featured Image</label><div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap"><input id="offerFeaturedImage" value="${attr(record?.image_url||'')}" placeholder="Image URL" style="flex:1"><button type="button" class="btn btn-light" onclick="pickFeaturedImage()">Upload Image</button></div><input id="featuredFileInput" type="file" accept="image/*" style="display:none" onchange="uploadFeaturedImage(this.files[0])"><div id="featuredPreview" style="margin-top:10px"></div></div>
      <div class="field"><label>Short Summary</label><textarea id="offerDescription" rows="4" placeholder="Short description for cards/search results">${escapeHtmlOffer(record?.description||'')}</textarea></div>
    </div>
    <aside class="offer-side">
      ...
      <div class="card" style="padding:16px;margin-top:14px"><h3 style="margin-top:0">Offer Settings</h3>
        <div class="field"><label>Category</label><select id="offerCategory"><option value="">Select category</option>${(cats.data||[]).map(x=>`<option value="${x.id}" ${String(record?.category_id||'')===String(x.id)?'selected':''}>${escapeHtmlOffer(x.name)}</option>`).join('')}</select></div>
        <div class="field"><label>Offer Type</label><select id="offerType"><option value="sale" ${selected(record?.offer_type,'sale')}>Sale / Offer</option><option value="warehouse" ${selected(record?.offer_type,'warehouse')}>Warehouse Sale</option><option value="clearance" ${selected(record?.offer_type,'clearance')}>Clearance</option><option value="promotion" ${selected(record?.offer_type,'promotion')}>Promotion</option><option value="freebie" ${selected(record?.offer_type,'freebie')}>Freebie</option></select></div>
        <div class="field"><label>Merchant / Shop</label><select id="offerMerchant"><option value="">No merchant</option>${(merchants.data||[]).map(x=>`<option value="${x.id}" ${String(record?.merchant_id||'')===String(x.id)?'selected':''}>${escapeHtmlOffer(x.name)}</option>`).join('')}</select></div>
        ...
        <div class="field"><label>Source / Official Link</label><input id="offerSource" type="url" value="${attr(record?.source_url||'')}" placeholder="https://..."></div>
      </div>
    </aside>
  </form><div id="offerSaveMsg" class="hint"></div>`;

  offerEditor=new Quill('#offerRichEditor',{theme:'snow',placeholder:'Write full offer details here...',modules:{toolbar:{container:[[{'header':[1,2,3,false]}],['bold','italic','underline','strike'],[{'list':'ordered'},{'list':'bullet'}],[{'align':[]}],['blockquote','link','image'],['clean']],handlers:{image:quillImageHandler}}}});
  if(record?.content_html)offerEditor.clipboard.dangerouslyPasteHTML(record.content_html);
  renderFeaturedPreview();
}

async function editOffer(id){const {data,error}=await db.from('deals').select('*').eq('id',id).single();if(error)return alert(error.message);openOfferEditor(data)}
...
async function deleteOffer(id){if(!confirm('Delete this offer?'))return;const {error}=await db.from('deals').delete().eq('id',id);if(error)alert(error.message);else showOffersCMS()}
function pickFeaturedImage(){document.getElementById('featuredFileInput').click()}
...
function prettyOfferType(v){return ({sale:'Sale / Offer',warehouse:'Warehouse Sale',clearance:'Clearance',promotion:'Promotion',freebie:'Freebie'})[v]||v||''}
...
function escapeHtmlOffer(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
