(async function(){
  ...
  const {data,error}=await client.from('categories').select('id,name,slug,icon,sort_order,active').eq('active',true).order('sort_order',{ascending:true}).order('name',{ascending:true});
  ...
  host.innerHTML=rows.map(c=>`<a class="category" href="deals.html?category=${encodeURIComponent(c.id)}"><div class="ico">${esc(c.icon||categoryIcon(c.name))}</div><b>${esc(c.name)}</b><small>View available offers</small></a>`).join('');
