(function(){
  ...
  ready(async function(){
    ...
    document.querySelectorAll('.drawer-head').forEach(head=>{
      const close=head.querySelector('.drawer-close');
      head.innerHTML='<a class="site-logo-link" href="index.html" aria-label="Ask Deal home"><img class="site-logo site-logo-drawer" src="'+logo+'" alt="Ask Deal"></a>';
      if(close) head.appendChild(close);
      else head.insertAdjacentHTML('beforeend','<button class="drawer-close" onclick="toggleDrawer(false)" aria-label="Close menu">×</button>');
    });
