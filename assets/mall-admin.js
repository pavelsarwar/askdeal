...
async function showMallManagement(){
  moduleCard.innerHTML=`<div class="deals-head"><div><h2 style="margin:0">Malls</h2><div class="result-count">Manage mall/location master data for offers.</div></div><button class="btn btn-primary" onclick="openMallEditor()">+ Add Mall</button></div><div id="mallRows">Loading...</div>`;
  const {data,error}=await db.from('malls').select('*').order('name');
  const host=document.getElementById('mallRows');
  if(error){host.innerHTML=`<p>${mallEsc(error.message)}</p>`;return;}
  host.innerHTML=`<div style="overflow:auto"><table class="admin-table"><thead><tr><th>Mall</th><th>City</th><th>Address / Landmark</th><th>Coordinates</th><th>Status</th><th>Actions</th></tr></thead><tbody>${(data||[]).map(r=>`<tr><td><b>${mallEsc(r.name)}</b></td><td>${mallEsc(r.city||'—')}</td><td>${mallEsc(r.address_text||'—')}</td><td>${r.latitude!=null&&r.longitude!=null?`${r.latitude}, ${r.longitude}`:'—'}</td><td>${r.active===false?'Inactive':'Active'}</td><td><button class="btn btn-light" onclick='openMallEditor(${JSON.stringify(r).replace(/'/g,"&#39;")})'>Edit</button> <button class="btn btn-light" onclick="deleteMall('${r.id}')">Delete</button></td></tr>`).join('')}</tbody></table></div>`;
}

function openMallEditor(record=null){
  mallEditingId=record?.id||null;
  moduleCard.innerHTML=`<div class="deals-head"><div><h2 style="margin:0">${record?'Edit':'Add'} Mall</h2><div class="result-count">Save mall location details once and reuse them in Add Offer.</div></div><button class="btn btn-light" onclick="showMallManagement()">← Back</button></div>
  <form onsubmit="saveMall(event)" class="admin-form">
    <div class="field full"><label>Name of Mall</label><input id="mallName" required value="${mallAttr(record?.name||'')}" placeholder="e.g. Mid Valley Megamall"></div>
    ...
    <div class="full"><button class="btn btn-primary">${record?'Update':'Save'} Mall</button></div><div id="mallSaveMsg" class="hint full"></div>
  </form>`;
}
...
async function saveMall(e){e.preventDefault();const name=document.getElementById('mallName').value.trim();const obj={name,google_maps_url:mallVal('mallMapsUrl'),address_text:mallVal('mallAddress'),city:mallVal('mallCity'),latitude:mallNum('mallLatitude'),longitude:mallNum('mallLongitude'),active:document.getElementById('mallActive').value==='true'};if(!mallEditingId)obj.slug=mallSlug(name)+'-'+Date.now().toString().slice(-5);let result;if(mallEditingId)result=await db.from('malls').update(obj).eq('id',mallEditingId);else result=await db.from('malls').insert(obj);if(result.error){document.getElementById('mallSaveMsg').textContent=result.error.message;return;}showMallManagement();}
async function deleteMall(id){if(!confirm('Delete this mall? Existing offers will keep working but lose the mall link.'))return;const {error}=await db.from('malls').delete().eq('id',id);if(error)alert(error.message);else showMallManagement();}
function mallSlug(v){return String(v||'mall').toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').slice(0,80)||'mall'}function mallVal(id){const v=document.getElementById(id)?.value?.trim();return v||null}function mallNum(id){const v=document.getElementById(id)?.value;return v!==''&&v!=null?Number(v):null}function mallAttr(v){return mallEsc(v).replace(/`/g,'&#96;')}function mallEsc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}